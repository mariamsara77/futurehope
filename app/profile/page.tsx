...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    getProfile()
      ...
      .catch((e) =>
        setError(
          e instanceof ApiError ? e.message : "Profile তথ্য আনা যায়নি।",
        ),
      )
  ...
  async function submit(e: FormEvent) {
    ...
    setError("");
    try {
      const p = await updateProfile(form, image);
      setProfile(p);
      setPreview(p.avatar_url || "");
      setImage(null);
      await refresh();
      setMessage("Profile সফলভাবে সংরক্ষণ হয়েছে।");
    } catch (e) {
      setError(
        e instanceof ApiError ? e.message : "Profile সংরক্ষণ করা যায়নি।",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function removeAvatar() {
    ...
    try {
      ...
      setPreview(p.avatar_url || "");
      await refresh();
      setMessage("Profile ছবি সরানো হয়েছে।");
    } catch (e) {
      setError(e instanceof ApiError ? e.message : "ছবি সরানো যায়নি।");
    }
  }
  ...
  if (!user)
    return (
      <main className="mx-auto max-w-3xl px-4 py-24 text-center sm:px-6">
        <div className="rounded-3xl bg-white p-10 shadow-sm ring-1 ring-zinc-200">
          <h1 className="text-3xl font-bold">Profile দেখতে লগইন করুন</h1>
          <p className="mt-3 leading-7 text-zinc-500">
            লগইন করলে আপনার ব্যক্তিগত তথ্য, যোগাযোগ ও profile photo এখান থেকে
    ...
    <main className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="rounded-3xl bg-zinc-950 p-7 text-white sm:p-10">
        ...
        <p className="mt-3 max-w-3xl leading-7 text-zinc-300">
          আপনার সংগঠনের profile তথ্য হালনাগাদ রাখুন। প্রয়োজন অনুযায়ী ভবিষ্যতে এই
          তথ্য member directory ও দায়িত্ব ব্যবস্থাপনায় ব্যবহার করা যাবে।
        </p>
      </div>
      <form onSubmit={submit} className="mt-7 space-y-7">
        ...
        <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-zinc-200 sm:p-8">
          <h2 className="text-2xl font-bold">মৌলিক তথ্য</h2>
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <Field
              ...
              onChange={setForm}
            />
            <ReadOnly label="ইমেইল" value={user.email} />
            <Field
              label="মোবাইল নম্বর"
              name="phone"
              value={form.phone}
        ...
        <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-zinc-200 sm:p-8">
          <h2 className="text-2xl font-bold">ঠিকানা ও পরিচিতি</h2>
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <Textarea
        ...
        {error && (
          <div
            role="alert"
            className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 font-medium text-red-700"
          >
            {error}
          </div>
        )}
...
function ReadOnly({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="rounded-2xl border border-zinc-200 bg-zinc-100 px-4 py-3 text-zinc-600">
        {value}
      </div>
    </div>
  );
